...
// This code is designed to investigate the p+gamma interaction threshold
// by specification of the CMS energy for the p+gamma process. 
...
//
// To run this program, select "cms-test.cxx" in the file "run-local.cc"
...
void user()
{
 ...
 Float_t energies[nsamples]={5.97e-8}; // Various gamma target energies in GeV

 Float_t ebeam=1e9; // Proton beam energy in GeV

 Float_t ecms=6; // CMS energy for p+gamma to test energy threshold
 ...
 Int_t evtlist=1;    // Produce an event listing of the 1st event of each sample (1) or not (0)

 Int_t fulldump=1;   // Include also the incoming particles, strings etc. in the event (1) or not (0)

 Int_t pystat=0;     // Produce Pythia statistics at the end of each sample (1) or not (0)
...
///// Float_t ebeam=0;
 ...
 for (Int_t k=1; k<2; k++)
 {
  ...
  for (Int_t isample=0; isample<nsamples; isample++)
  {
   ...
   {
    jrun=-jrun;
//    gen->Init("free","p","gamma",0,&pbeam,&ptarget,wxsec);
    gen->Init("cms","p","gamma",ecms);
   }
   ...
   {
    if (evtlist && !ievt) // Produce Pythia listing for the 1st event of each sample
    {
     if (!fulldump)
     {
